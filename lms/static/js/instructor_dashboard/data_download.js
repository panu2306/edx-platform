...
(function() {
    ...
    DataDownload = (function() {
        function InstructorDashboardDataDownload($section) {
            ...
            this.$async_report_submit_btn.click(function (e) {
                var errorMessage = '';
                var data = {};
                var selectedOption = dataDownloadObj.$grade_reports_selector.find(':selected');
                var url = selectedOption.data('endpoint');
                var verifiedReport = selectedOption.data('verified_learners_only');
                if (selectedOption.data('verified_learners_only') !== undefined){
                  data['verified_learners_only'] = verifiedReport
                }
                var targret_name = selectedOption.val();
                dataDownloadObj.clear_display();
                return $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    ...
                    error: function(error) {
                        if (error.responseText) {
                            ...
                            errorMessage = dataDownloadObj.get_error_by_target_name(targret_name);
                        }
                        dataDownloadObj.$reports_request_response_error.text(errorMessage);
                        return dataDownloadObj.$reports_request_response_error.css({
                            display: 'block'
        ...
        InstructorDashboardDataDownload.prototype.get_error_by_target_name = function (target_name) {
          var errorMessage = '';
          if (target_name === 'calculate-grades-csv') {
              errorMessage = gettext('Error generating grades. Please try again.');
          }
          else if (target_name === 'problem-grade-report') {
              errorMessage = gettext('Error generating problem grade report. Please try again.');
          }
          else if (target_name === 'export-ora2-data') {
